...
**Diagram Types:** architecture-beta (cloud/infra), C4 Model, Component, Layered, Microservices, Event-Driven

---

## Choosing the Right Diagram Format

| Use Case | Format | Renders in repos? |
|----------|--------|-------------------|
| All architecture diagrams embedded in repo markdown | `flowchart TD` / `graph TB` | ✅ Yes — GitHub, GitLab, Confluence |
| PNG export via mmdc only (not embedded in markdown) | `architecture-beta` | ✅ mmdc only — ❌ breaks on GitHub/GitLab |

**Default: `flowchart TD` for everything.** Use `architecture-beta` only when the explicit goal is mmdc PNG generation. It requires Mermaid v11.1.0+ and iconify registration — GitHub and GitLab do not support it.

---

## architecture-beta (mmdc/PNG Export Only — NOT for repo markdown)

`architecture-beta` renders real graphical icons natively. It does **not** support `classDef` — do not add styling. Visual organization comes from groups instead.

### Icon Tiers (use in order of preference)

**Tier 1 — Built-in icons (always render everywhere):**
```
cloud  database  disk  internet  server
```

**Tier 2 — Iconify `logos` pack (renders in Mermaid Chart and mmdc; may not render in GitHub/GitLab):**
```
logos:aws-ec2         logos:aws-rds          logos:aws-lambda
logos:aws-aurora      logos:aws-s3           logos:aws-cloudfront
logos:aws-api-gateway logos:aws-sqs          logos:aws-sns
logos:aws-elb         logos:aws-fargate      logos:aws-cloudwatch
logos:google-cloud-run  logos:google-cloud-sql  logos:google-cloud-storage
logos:azure-functions   logos:azure-sql-database  logos:azure-storage-blob
logos:kubernetes      logos:docker           logos:redis
logos:postgresql      logos:nginx            logos:kafka
```

**Fallback rule:** If rendering in a context that may not support iconify (GitHub README, GitLab, Confluence), use Tier 1 built-in icons only, or fall back to `flowchart TD` with Unicode emoji and classDef.

### Basic Syntax

```mermaid
architecture-beta
    group api(cloud)[Payment API]

    service gateway(internet)[API Gateway] in api
    service processor(server)[Payment Processor] in api
    service db(database)[Payments DB] in api
...
### With Iconify Icons (mmdc / Mermaid Chart)

```mermaid
architecture-beta
    group api(logos:aws-lambda)[Payment Service]

    service gateway(logos:aws-api-gateway)[API Gateway] in api
    service processor(logos:aws-fargate)[Payment Processor] in api
    service db(logos:aws-aurora)[Payments DB] in api
...
### Multi-Group Pattern (AWS 3-Tier)
...
architecture-beta
    ...
    service alb(logos:aws-elb)[Load Balancer] in edge
    service api(logos:aws-fargate)[API Service] in app
    service worker(logos:aws-fargate)[Worker] in app
    service db(logos:aws-aurora)[Aurora PostgreSQL] in data
    service cache(logos:redis)[ElastiCache] in data
...
db:R --> L:server         # directed (right-to-left)
db:R <-- L:server         # directed (left-to-right)
db:T -- B:server          # 90-degree edge (top to bottom)
```
...
## flowchart TD — Logical / Abstract Architecture

Use `flowchart TD` when the diagram is conceptual rather than infrastructure-based. This format supports `classDef` with the mandatory high-contrast palette.

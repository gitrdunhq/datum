...
2. [Mermaid 11 Incompatibilities](#mermaid-11-incompatibilities)
3. [Cross-Cutting Issues (All Diagrams)](#cross-cutting-issues-all-diagrams)
...
## Mermaid 11 Incompatibilities

### ❌ Mermaid 11 Bug A: Unicode Arrows Inside Node Labels

**Severity:** 🔴 Critical — Causes immediate parse failure

**Problem:** Mermaid 11's tokeniser treats `→`, `←`, `⇒` and other Unicode arrow characters as edge operators, even when they appear inside node label text (`[...]`, `(...)`, `{...}`). After a closing `)`, the parser sees `CYLINDEREND → PS` and panics.
...
**Rule:** Never use `→`, `←`, `⇒`, `⟹`, or any Unicode arrow inside node label text. Use `to`, `returns`, `·`, or plain `-` instead. The arrow restriction applies between nodes only.

**Diagram Types Affected:** All diagram types in Mermaid 11+

---

### ❌ Mermaid 11 Bug B: `<br/>` Inside Subgraph Title Strings

**Severity:** 🟠 High — Subgraph title renders as literal text including the tag

**Problem:** `<br/>` HTML entities work inside node labels (`[...]`) but are not supported inside subgraph title strings. Mermaid 11 treats subgraph titles as plain strings.

**Incorrect:**
```mermaid
graph TB
    subgraph SG ["Auth Service<br/>Core Module"]
        A --> B
    end
```
...
```mermaid
graph TB
    subgraph SG ["Auth Service — Core Module"]
        A --> B
    end
```

**Rule:** Keep subgraph titles to one line. Use ` — `, `: `, or a space to separate words. If you need a two-line label inside the subgraph, add a dedicated title node instead.

**Diagram Types Affected:** All diagram types using `subgraph` in Mermaid 11+

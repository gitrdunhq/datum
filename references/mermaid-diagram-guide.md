...
**Granularity Guidelines (C4 Model):**
- **Level 1: System Context** → Use `C4Context` for stakeholder views and system boundaries.
- **Level 2: Container** → Use Architecture/Deployment diagrams for high-level technical choices (API, DB, Worker).
- **Level 3: Component** → Use Flowcharts/Sequence diagrams for internal logic and service interactions.
- **Level 4: Code** → Use Class diagrams only when internal implementation details are critical.

### 2. Layout and Flow Direction

- **Left-to-Right (LR)**: Use for sequential processes like pipelines, timelines, and data flows. Readers naturally expect time to flow horizontally.
- **Top-to-Bottom (TD/TB)**: Use for hierarchical relationships like org charts, architecture layers, and decision trees.
- **Vertical preference**: Use TD for diagrams with many branches or long labels (easier to read on standard screens).

### 3. Labeling Conventions

- **Node Labels (Nouns)**: Use **Title Case** and noun phrases (e.g., `[Auth Service]`, `[User Database]`).
- **Edge Labels (Verbs)**: Use **lowercase** and verb phrases (e.g., `|validates token|`, `|sends request|`).
- **Concision**: Keep edge labels under **5 words**. If more context is needed, use a `Note`.
- **Meaningful IDs**: Assign semantic node IDs (e.g., `AuthService` instead of `A`) so the raw code is maintainable.

### 4. Keep Diagrams Focused

**Threshold**: A diagram with > 15-20 nodes loses its communication value.

**Strategies:**
- **Subgraphs**: Group related nodes logically (e.g., "Frontend", "Data Layer").
- **Multiple Views**: Create one high-level overview and separate detail diagrams for specific components.
- **Simplicity**: Remove decorative elements. If an element doesn't improve understanding, delete it.

### 5. Mermaid Code Organization (Maintainability)

To ensure diagrams are maintainable and readable by both humans and LLMs, follow these code-level patterns:

#### A. Declare Nodes First
Always list all nodes with their shapes and labels at the top. Use ONLY their IDs when defining relationships later.
...
#### B. Use Semantic Identifiers
Avoid `A`, `B`, `C`. Use descriptive IDs that match the component name.

- ✅ `AuthService[Authentication Service]`
- ❌ `A[Authentication Service]`

#### C. Logical Grouping & Comments
Use `%%` comments and whitespace to separate different flows or architectural layers.
...
#### D. Branch Alignment
When a single node branches into multiple paths, align the source code horizontally to make the branching logic obvious.
...
#### E. Shared Node Clarity
If multiple nodes depend on a shared component, define the primary flow clearly on its own line.

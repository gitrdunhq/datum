...
| `build_command` | unset | Optional, alongside `test_command`: when set, an independent `build-verify` step (e.g. `pnpm typecheck`, `go build ./...`) runs in the same batch as the post-GREEN test-verify and again in Validate, failing the lane `build_verify_failed`/`build_verify_unavailable` on the same fail-closed shape as `green_verify_failed`; unset (the default) means the step is never emitted and nothing changes. |
...
**Lane kinds** (`kind` in tasks.json / lane-plan.json): `behavioral` runs the full RED→GREEN→REFACTOR lane; `structural` goes straight to REFACTOR; `kind: "integration"` is a `task-INT-<n>` lane Plan synthesises from PROPERTIES.md's `## Integration Invariants` table, RED-only, scheduled after every task its invariants cover. Plan's gate names `invariant_missing_for_question` (an answered QUESTIONS id with no invariant row), `invariant_covers_unknown_task` (a Covers entry naming no task) `no_integration_invariants` (the table is absent or empty and no INT lane exists) and `int_lane_without_invariant` (an INT lane exists but no invariant row backs it). `integration_failed` is the lane runner's verdict when an integration lane's independent post-RED verify comes back red — it names the covered task ids and invariant ids and fails the lane at RED (no GREEN, skeptic or REFACTOR); triage classifies it `code_defect` and routes it to the consumer, never datum's own tracker. That verify runs the lane's own test files when `test_command` is pytest- or vitest-shaped (the files are appended as arguments); any other command runs the whole suite and the lane logs `integration_verify_unscoped`, so an unrelated red would read as `integration_failed` — Validate owns the whole-suite run. The plan gate also warns `plan_not_sliced` when no task crosses a layer boundary: a task is a shippable, testable slice cut through every layer, not a module (a warning until one epic has run under it).

...
7. **CLI Commands Only:** Every pipeline operation an agent runs against this repo (gate checks, lane-state read/write, skeleton creation, commit-queue, test-count-gate, worktree setup/merge/cleanup, closeout, bugfile, etc.) MUST go through its `datum <command>` subcommand — never a raw `python scripts/*.py`, `uv run python -m datum...`, or hand-rolled shell equivalent. The CLI is the single, tested entrypoint (see `datum --llms-txt` for the full command tree); bypassing it skips the argument validation, config loading, and gate contracts those subcommands enforce. If a needed operation has no `datum` subcommand yet, that's a gap to fix in the CLI (`datum dev ...` or a new subcommand) — not a reason to shell out around it.

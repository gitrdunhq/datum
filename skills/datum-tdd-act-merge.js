...
function parseBatchResult(raw, steps2) {
  ...
  if (results.length < steps2.length && !stoppedByFailFast) {
    const returned = new Set(results.map((r) => r.name));
    const absent = steps2.map((s) => s.name).filter((n) => !returned.has(n));
    return {
      steps: [],
      failed: null,
      missing: true,
      scriptError: `batch_incomplete: ${results.length} of ${steps2.length} step records returned and no non-tolerant failure stopped the batch \u2014 the runner returned a partial result (last record: ${last?.name ?? "none"}); absent: [${absent.join(", ")}]`
    };
  }

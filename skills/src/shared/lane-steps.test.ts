...
import {
  ...
  readContextSteps,
  contextFromSteps,
  CONTEXT_FILE_RELAY_LIMIT_BYTES,
} from './lane-steps'
...
// ---------------------------------------------------------------------------
// readContextSteps / contextFromSteps — determinism fix for datum-refine.ts
// and datum-properties.ts, which used to hand an LLM `reader` agent
// util-read-context.md and trust its echoed JSON verbatim (a 90 KB relay came
// back as 6.7 KB of "successful" abridged content in dogfooding). Mirrors the
// batched cat + wc -c + Buffer.byteLength verification already applied to
// datum-plan.ts's context_files relay (commit a7093d2).
// ---------------------------------------------------------------------------

function fakeResult(stepMap: Record<string, string | null>): { steps: { name: string; exit_code: number; stdout: string; stderr: string }[]; failed: null; missing: boolean } {
  return {
    steps: Object.entries(stepMap)
      .filter(([, v]) => v !== null)
      .map(([name, stdout]) => ({ name, exit_code: 0, stdout: stdout as string, stderr: '' })),
    failed: null,
    missing: false,
  }
}

describe('readContextSteps', () => {
  it('always leads with branch and epic-dir, derived the same way actStartSteps does', () => {
    const steps = readContextSteps({ files: [] })
    expect(names(steps)).toEqual(['branch', 'epic-dir'])
    expect(steps[0].command).toContain('git rev-parse --abbrev-ref HEAD')
    expect(steps[0].command).toContain('__eb=')
    expect(steps[1].command).toContain('$__eb')
    expect(steps[1].command).toContain('docs/epics/')
  })

  it('adds a cat + wc -c pair per file, in file order, all tolerant', () => {
    const steps = readContextSteps({ files: ['TICKET.md', 'SPEC.md'] })
    expect(names(steps)).toEqual(['branch', 'epic-dir', 'ctx-cat-0', 'ctx-wc-0', 'ctx-cat-1', 'ctx-wc-1'])
    expect(steps.every((s) => s.tolerant)).toBe(true)
    expect(steps.find((s) => s.name === 'ctx-cat-0')!.command).toContain('TICKET.md')
    expect(steps.find((s) => s.name === 'ctx-wc-0')!.command).toContain('wc -c')
  })

  it('falls back to a NOT_FOUND marker / -1 byte count when the file is absent', () => {
    const steps = readContextSteps({ files: ['MISSING.md'] })
    expect(steps.find((s) => s.name === 'ctx-cat-0')!.command).toMatch(/__DATUM_CTXFILE_NOT_FOUND__/)
    expect(steps.find((s) => s.name === 'ctx-wc-0')!.command).toMatch(/-1/)
  })

  it('appends extraCommands after the file steps, in order, tolerant', () => {
    const steps = readContextSteps({
      files: ['TICKET.md'],
      extraCommands: [
        { name: 'agent-types', command: `jq -r '.agent_types // true' .datum/config.json` },
        { name: 'spec-exists', command: `test -f "$__eb/SPEC.md" && echo true || echo false` },
      ],
    })
    expect(names(steps)).toEqual(['branch', 'epic-dir', 'ctx-cat-0', 'ctx-wc-0', 'agent-types', 'spec-exists'])
    expect(steps.find((s) => s.name === 'agent-types')!.tolerant).toBe(true)
    expect(steps.find((s) => s.name === 'spec-exists')!.tolerant).toBe(true)
  })
})

describe('contextFromSteps', () => {
  it('derives branch and epicDir from the branch/epic-dir steps', () => {
    const r = fakeResult({ branch: 'datum/x', 'epic-dir': 'docs/epics/datum/x' })
    const out = contextFromSteps(r as never, [])
    expect(out.branch).toBe('datum/x')
    expect(out.epicDir).toBe('docs/epics/datum/x')
    expect(out.contents).toEqual({})
    expect(out.warnings).toEqual([])
  })

  it('pairs each ctx-cat-<i> with its ctx-wc-<i> by index and returns verified contents', () => {
    const content = 'hello world'
    const r = fakeResult({
      branch: 'b', 'epic-dir': 'docs/epics/b',
      'ctx-cat-0': content, 'ctx-wc-0': String(Buffer.byteLength(content, 'utf8')),
    })
    const out = contextFromSteps(r as never, ['TICKET.md'])
    expect(out.contents['TICKET.md']).toBe(content)
  })

  it('maps a NOT_FOUND marker / -1 byte count to null, not an error', () => {
    const r = fakeResult({
      branch: 'b', 'epic-dir': 'docs/epics/b',
      'ctx-cat-0': '__DATUM_CTXFILE_NOT_FOUND__', 'ctx-wc-0': '-1',
    })
    const out = contextFromSteps(r as never, ['MISSING.md'])
    expect(out.contents['MISSING.md']).toBeNull()
    expect(out.warnings).toEqual([])
  })

  it('throws context_relay_mismatch naming the path and both byte counts on a mismatch', () => {
    const r = fakeResult({
      branch: 'b', 'epic-dir': 'docs/epics/b',
      'ctx-cat-0': 'short', 'ctx-wc-0': '999',
    })
    expect(() => contextFromSteps(r as never, ['SPEC.md'])).toThrow(
      /context_relay_mismatch: SPEC\.md expected 999 bytes, got 5 bytes/,
    )
  })

  it('omits files over the 64KB relay cap with a warning instead of relaying an abridged copy', () => {
    const big = 'x'.repeat(100)
    const declaredOversize = CONTEXT_FILE_RELAY_LIMIT_BYTES + 1
    const r = fakeResult({
      branch: 'b', 'epic-dir': 'docs/epics/b',
      'ctx-cat-0': big, 'ctx-wc-0': String(declaredOversize),
    })
    const out = contextFromSteps(r as never, ['HUGE.md'])
    expect(out.contents['HUGE.md']).toBeNull()
    expect(out.warnings.join(' ')).toMatch(/HUGE\.md omitted/)
    expect(out.warnings.join(' ')).toMatch(/exceeds relay limit/)
  })

  it('runs end-to-end under bash: branch/epic-dir threading plus a real cat + wc -c pair', () => {
    const dir = mkdtempSync(join(tmpdir(), 'datum-readctx-'))
    try {
      writeFileSync(join(dir, 'TICKET.md'), 'ticket body\n')
      const steps = readContextSteps({ files: ['TICKET.md', 'MISSING.md'] })
      const script = batchScript(steps)
      const raw = execFileSync(
        'bash',
        ['-c', `cd ${JSON.stringify(dir)} && git init -q && git checkout -q -b datum/e && git commit --allow-empty -q -m x && ${script}`],
        { encoding: 'utf8' },
      )
      const r = parseBatchResult(raw, steps)
      expect(r.failed).toBeNull()
      expect(stepStdout(r, 'branch')).toBe('datum/e')
      expect(stepStdout(r, 'epic-dir')).toBe('docs/epics/datum/e')
      const out = contextFromSteps(r, ['TICKET.md', 'MISSING.md'])
      expect(out.contents['TICKET.md']).toBe('ticket body\n')
      expect(out.contents['MISSING.md']).toBeNull()
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }

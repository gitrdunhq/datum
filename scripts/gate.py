...
def gate_plan(yolo: bool, config: dict) -> None:
    ...
    schema_errors = validate_payload(assets_dir() / "schemas/lane-plan.schema.json", lane_plan_path)
    if schema_errors:
...
def gate_validate_packets(config: dict) -> None:
    ...
    for packet_path in packets_dir.glob("*.json"):
        packet_errors = validate_payload(assets_dir() / "schemas/packet.schema.json", packet_path)

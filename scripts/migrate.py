...
def main() -> None:
    ...
    state = load_state()
    if not state:
        print(json.dumps({"ok": True, "skipped": True, "reason": "no state.json"}))
        return
    ...
    try:
        ...
        errors = validate_value(STATE_SCHEMA, migrated)
    except Exception as exc:  # pragma: no cover - defensive command surface
        errors = [str(exc)]

    if errors:
        print(json.dumps({"ok": False, "changes": changes, "errors": errors}, indent=2))
        sys.exit(1)

    print(json.dumps({"ok": True, "dry_run": args.dry_run, "changes": changes}, indent=2))

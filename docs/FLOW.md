...
2. **Candidates from the claude-skills audit** — `docs/research/claude-skills-audit-20260906-130507.md` audits all 43 randommonicle/claude-skills guardrails against this file (one agent per skill, rendered by `scripts/render_skills_audit.py`). Nothing scored adopt-now; fourteen scored adopt-later, led by `prove-it-can-fail` (a standing gate that every named check can go red: revert the implementation, run the lane's tests, require a failure), `one-real-ride` (#457, the browser gate), `mass-red-triage` (Validate records the test COUNT beside the exit code and fails `validate_count_regressed` when a green run collected fewer tests than the epic base), `parallel-work-recon` (re-read the epic tip before the squash fold and name `epic_tip_moved`), `reproduce-the-real-build` (a `build_command` beside `test_command`, `build_verify_failed`), and `verified-citations` (a Review finding's file:line is checked against the tree, `citation_unverified`). Thirteen were already covered by named failures and closures below; the rest do not apply to a TDD pipeline.

### Closed

- **The docs tree described a pipeline that was never built** — closed in e31c8d89..dd9dfdad (audit: `docs/research/docs-audit-20260906-152916.md`). Both README pointers into `docs/` landed on dead files: the pre-implementation design spec (a state.json, a TOML config, a commit queue, a PR phase, a test-redaction script) presented as the reference, and flowcharts with an Architect gate and Properties "skippable in yolo". Nine files deleted (including a second CURRENT_STATE.md that told agents source lives in `src/datum/`, a memory dump from another project whose GREEN rule contradicts `green_edited_tests`, and `analyze_properties.py`, a producer with no caller), one archived, three corrected. `datum landscape` now prints directory lines and follows `git ls-files` inside a repo (210ae868), so a plain walk over `.temp`, `.tokensave` and graph dumps can no longer turn a 500-line doc into 3600. Rule: a doc that names a mechanism must name the code that enforces it; this file is the yardstick and README links only it and its rendered diagrams.
- **A GREEN blocked on the lane's own test needed an operator to steer the lane** — closed in 9d20096b/fc82d2bc (#440; elonchesd epic-2 task-006, then player-guidance task-011). GREEN reported blocked with `needs_write` naming the lane's own test file because the RED fixture encoded a precondition that cannot hold; its diagnosis was exact both times and it was rightly forbidden to edit the test. When every `needs_write` path is one of the lane's own tests, RED now runs once more carrying that diagnosis, the post-RED gates re-run on the repair, and GREEN runs once more; a repair that does not land is `red_repair_failed`, a second block by the same shape is `green_blocked_needs_write` naming the repair. The stage that may edit the file is the one asked to fix it, with the other stage's evidence in hand.
- **Writers of a shared file ran as siblings and conflicted at squash; a mistyped script was mistyped again by the same model class** — closed in a8f17108 and 13176623/9eb2b930 (elonchesd player-guidance wf_6cb9491b-36b). Three screen lanes each extended `src/rules/copy.ts`; every later claimant depended only on the first, so they forked from the same base and conflicted with each other. Writers of a shared file are now chained in task order, so each merges the previous writer's branch at intake and squash order equals write order. Separately, `batch_script_corrupt` twice per lane with the same quote dropped in the same place: a fast runner repeats its slips, so the corrupt retry goes to the balanced model. Elonchesd's "deliver the script by path" idea is right and impossible here for the same reason as caliper's result-file idea: the Workflow vm has no filesystem, so anything a runner executes has been relayed by a runner.
- **Self-hosting datum on the integration-lanes epic surfaced four environment defects in one evening** — closed in 991fd266/20847480, 0ec5eaff/19bdfe10, 630429b4/3c67b983 (wf_c296b6b0-721, wf_2749a43b-680, wf_96fa4660-133). The runner's shell lacked the homebrew prefix and jq, so the batch wrapper appends the tool prefixes and names a missing jq (`batch_tool_missing`); the decomposer listed a generated bundle beside its source, so lane-plan rejects `@generated` files and the decompose prompt forbids them; the developer's global git hooks fired inside lane worktrees and a post-checkout graph rebuild left unmerged index entries, so hooks are off per scratch worktree through per-worktree config; and `uv sync --frozen` gave lanes an env without the extras the main checkout has, so `worktree_sync_args` in the datum config names the installer arguments. One rule behind all four: the pipeline runs in the operator's environment, not a clean room, and every assumption about that environment is a named check or a named failure.
- **An empty verify reply was reported as a red suite and filed as "GREEN lied"** — closed in 14948947/334e8739 (elonchesd datum/player-guidance wf_dee84cc2-e64 task-001). The post-GREEN test-verify batch returned nothing; the lane turned exit=null into `green_verify_failed`, triage classified it as agent behaviour, and 17 dependents were blocked while the committed GREEN passed 434/434 in a scratch worktree. `runBatch` re-sends an empty reply once, like a refusal or a mistyped script, and names a second one `runner_empty_result`; the lane never emits green_verify_failed without a parsed exit code, a null exit at any of the three GREEN verify sites is `green_verify_unavailable`, and the classifier files it as infrastructure. Same family as `count_gate_no_output` and `ownership_check_failed`: a check that did not run has no verdict, and "null" is never "failed".
- **The plan gate failed on refine's Assumption Audit after twenty minutes of planning agents** — closed in 123935d8/8dedcc06/c7275669 (elonchesd datum/player-guidance wf_251cf8a3-363). Rows whose Resolves read "Q8 (partially)" and "Q2 (related)" were rejected by an exact-match rule although each names an answered question, the check ran only at the plan gate although the table is refine's output, and the halt said to fix tasks.json. A guess now resolves by any answered Q<N> in the cell and the message names SPEC.md; the refine gate runs the audit itself, where a fix costs seconds; and plan runs refine's structural gate right after Read, halting as `plan_prerequisite_failed` before any planning agent. Principle: a check on a phase's output belongs to that phase's gate, and every gate that runs after expensive work is a backstop, never the first line.
- **A read witness with ten correct hex chars and one dropped digit failed a valid RED** — closed in 65749b10/ef49f2e1 (caliper eedom wf_751ea0e4-653 task-002, #566). The RED agent committed eight sound tests and cited `8cc9785049da` for blob `8cc97850499d…`; the exact-prefix rule rejected it, the lane failed `context_read_unverified` and four descendants were blocked. The proof is the leading run: ten correct hex chars cannot come from anywhere but the hash, the same standard as BUG K2's seven-char floor, so a value whose leading seven or more hex chars match is accepted when it diverges afterwards, reported as a near miss and logged `read_witness_near_miss`; fewer than seven correct leading chars is still unverified. Fourth witness lesson (K, K2, K3, and this): verify the unforgeable content and tolerate every shape and slip around it.
- **A 24 KB batch through the fast runner was corrupted and never retried** — closed in f8e38bed/db63b130 (caliper eedom wf_8c7ccdb5-11d, #566). The lane-plan build script came back with " on purpose" inserted into a description string; the sha guard named `batch_script_corrupt`, but the journal shows one attempt: that site, like 28 others, called `agent(batchCommandPrompt(...))` directly and never went through `runBatch`, so the refusal retry and the corrupt-script retry existed and did not apply to it. Now every batch in every script and in the tracker goes through `runBatch` (a static tripwire forbids the direct call), and a batch whose prompt exceeds 8 KB is routed to the balanced model and says so, since at that size a fast-model transcription slip is near-certain and a retry would mostly fail twice. Same shape as the ast-grep finding: a safeguard that most sites bypass is a safeguard for none of them; the tripwire is what makes it universal.
- **The runner rewrote a relayed file in transit and the phase halted on it** — closed in 9e05ec9f/06d41d4c (caliper eedom wf_9bf2c994-801, #566). The datum-cli runner transcribes the batch RESULT as it does the script: one contiguous 340-byte span vanished from the middle of an 8.4 KB QUESTIONS.md while the 6 KB TICKET beside it came back intact, refine halted with `context_relay_mismatch`, and the corrupted result was a journal row a resume replayed. Caliper's suggestion (write the result to a file, return only a path and hash, let the workflow read the file) is the right instinct and impossible here: the Workflow vm has no filesystem, so every byte the script holds has crossed a runner. What can be done: verify by the probe's blob sha rather than the byte count (a same-length corruption passes a count), re-fetch a mismatched inline batch once with a fresh runner under a distinct prompt (so a resume does not replay the row), and defer what still mismatches to the consuming agent with path, bytes and a mandatory witnessed Read, the route every over-budget file already takes. A mismatch is no longer a halt at any of the four relay sites.
- **Repro files a skeptic left behind failed the next stage's suite** — closed in df2975fc/39eb52b1 (caliper eedom wf_fa38ac24-890 task-005). Four untracked scratch tests written while a skeptic lens reproduced its findings were collected by REFACTOR's mandated test run; two monkeypatched `os.chdir` and left the process in a deleted tmp_path, a later relative-path test failed deterministically, and REFACTOR reported "blocked" on a pristine GREEN commit. Every stage commits its work, so anything untracked between stages is a stray: `strayCleanSteps` lists it, removes it, confirms, and the lane names it (`stray_untracked_files`, or `stray_clean_unchecked` when the batch did not run); it rides in the post-GREEN verify batch and runs once more before REFACTOR. The skeptic prompt forbids creating files in the worktree. Principle: a verify runs against the committed tree, and a rule in a prompt is a proposal that the batch enforces. Caliper's two follow-ups landed in cdc13b0e/788b2d00: the sweep leaves `.datum/` and `.temp/` (sanctioned scratch; the lane's own spec file lives under `.datum/`), and a REFACTOR that reports blocked or failed WITHOUT a commit is `refactor_skipped`, the tree reset to HEAD and verified independently, green completing the lane; the old handling failed the lane as agent behaviour on a pristine GREEN commit and cost a full re-run of the batch. An optional stage's honest "could not" is not the lane's failure; only its committed tree is judged.
- **The placeholder scan read the whole owned file, strings included** — closed in 440aea1e/ff009375 (caliper eedom wf_0837ad8b-e5f task-005). `raise NotImplementedError` at line 240 of a test file was inside a `textwrap.dedent("""...""")` of Python source a pre-existing test feeds to the indexer, on a line the RED commit never touched; the lane halted as `placeholder_assertions` and triage filed it as agent behaviour. The scan now runs on a filtered copy: only lines the lane added since its merge-base survive, with line numbers kept, and the inside of any multi-line string is blanked; ast-grep already parsed strings correctly, the grep fallback eedom ran did not. Third instance of BUG I's lesson (a token matched inside a string) and the first of a new one: a gate on the lane's work must look only at the lane's diff, never at what the file already held. Caliper then noticed ast-grep was on eedom's PATH while the halt was grep output: ast-grep exits 1 for "no match" and for every error alike, so `ast-grep || grep` ran the fallback on every clean file and the parse-aware verdict had never been trusted since the check was written (bceded3c/f23da7af: trusted when present and silent on stderr). Trusting it exposed that the TS whole-body shapes never matched the real skeleton, whose `// Assert` comment the exact shape excludes; the TS placeholder is the skeleton's literal throw. Lesson: a fallback that fires on the primary's normal negative answer replaces the primary silently; test the primary's verdict on its own. Caliper's ledger note: the guard reads the lane plan's declared test files, not the files the RED commit touched, so moving the appended test to another file did not take the declared file out of scope; with the added-lines filter a declared-but-untouched file has no added lines and is clean, and a touched-but-undeclared file is the ownership check's finding, not this guard's.
...
- **Second eedom epic to closeout** — caliper #562 on eedom, 2026-09-05, wf_2fb62759-699 on 788b2d00's predecessor bundle (86577e5a): Act (task-005 REFACTOR passed on a clean worktree), Validate, Review (8 findings, 0 high), Closeout archived e44fee4 without a halt; 6 of the 8 findings fixed post-pipeline RED-first (284b80d), all 8 decisions recorded via `datum review-accept` (8e564e1), suite 3731 green. Seven launches, each stopped by one thing: (1) QUESTIONS.md unanswered; (2) relay trailing-newline drop (646ee9ab); (3) gate batch refused by the host classifier; (4) the runner re-typed a script (d71dfb8d); (5) act setup refused twice on the new bundle, and the resume replayed the cached refusal; (6) a task-004 skeptic loop, then the task-005 placeholder guard (440aea1e/bceded3c); (7) REFACTOR "blocked" on stray files (df2975fc/cdc13b0e). Four of the seven were datum defects, all closed above; two were the operator's permission rules; one was an unanswered question.
- **Third eedom epic to closeout** — caliper #566 on eedom, 2026-09-06, wf_7fe6f868-6e1 on ef49f2e1-era bundles: Act (6 lanes; two corrupted batches caught by the sha guard and retried successfully; stray-clean refusals tolerated), Validate, Review (3 findings, 2 high, all three fixed post-review in 200cce2 with decisions recorded in 1973e5a), the resume passed the gate and Closeout archived de6fdc0 with an accurate CURRENT_STATE.md because closeout ran after the decisions existed (the #460 shape, avoided by ordering). Five launches: (1) unanswered questions; (2) relay mid-file deletion (9e05ec9f); (3) lane-plan `batch_script_corrupt` with no retry (f8e38bed); (4) stopped by the operator to pick up f8e38bed; (5) witness typo (65749b10), then the clean run. Observation for the record: the skeptic caught task-001 mirroring an existing fnmatch matcher although the answered Refine question specified path-aware globstar, and Review did not; the answer sat in the lane spec and the GREEN agent copied the neighbouring code instead. A property derived from each answered QUESTIONS.md item would have made that a RED failure (#461).
...
- `Workflow({resumeFromRunId})` replays every `agent()` whose prompt and options are unchanged from the journal, including one that failed, was refused by the host classifier, or returned nothing. A resume therefore never retries a failed lane or a refused batch; the retry markers (`:retry` label, "attempt 2 of 2" line) only make the retry itself a distinct call. A fresh launch is the only way to re-run a refused or failed agent (caliper #562 launch 5).

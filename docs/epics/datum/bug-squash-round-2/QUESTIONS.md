...
[Answer]: Confirmed — one lane per bug (A1). They're disjoint files; grouping would only reduce parallelism for no benefit.
...
[Answer]: Confirmed — regression test + gitnexus_detect_changes() scoped-change check is sufficient (A2). No full pipeline smoke run required per-lane.
...
[Answer]: Confirmed — fully parallel, no ordering constraint (A3). This epic itself doesn't depend on #213 landing first (we worked around it manually via `datum init`).
...
[Answer]: Go with the broad interpretation (A5) — directory/package-based test conventions generically (Swift SPM's `Package.swift` + `Tests/<Target>/` was the observed case in #270's originating issue, but the fix shouldn't be Swift-specific).
...
[Answer]: Confirmed — hard-fail (A6). Burning agent tokens on a doomed lane is worse than a false-positive block the operator can override by fixing lane-plan.json.

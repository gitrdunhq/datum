...
| INT-006 | The sentinel `{"status": "no_state_available"}` written by `collect_tasks.py`/`collect_token_metrics.py` (task-010) survives unmodified through `collate.py`'s rendering into `closeout-data.json`'s `tasks` field as a non-null, non-zero value | task-009, task-010 | spec:9 |

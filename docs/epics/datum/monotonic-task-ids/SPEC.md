...
4. **Integration lanes synthesized by `build_lane_plan` take sequential `<PREFIX>-<n>` ids from the same counter, not the `task-INT-` shape.**
   ...
   - AC: `tests/test_integration_invariants_frontier.py` and `tests/test_lane_plan_integration_lanes.py` continue to pass unmodified, and `tests/test_lane_plan_schema_int_ids.py` passes with only the change Assumption 8 sanctions (its `TASK-001` fixture moves from the rejected to the accepted side), verifying that epics already using `task-INT-\d+` ids remain valid. (Wording updated 2026-09-08 per closeout FU-3.)

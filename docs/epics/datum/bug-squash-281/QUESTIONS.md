...
[Answer]: Yes, use the `worktrees_app` sub-app pattern — add a `lane_state_app` Typer sub-app with `read`/`write` verbs sharing `--epic`/`--task` options, mirroring `worktrees_app`'s structure since lane-state operations are conceptually the same family (per-epic, per-lane bookkeeping) as worktree lifecycle ops.
...
[Answer]: Assign to all applicable root groups — a blocked lane with multiple failed ancestors (diamond dependency) should appear under each failed root's group. Triage is for human diagnosis; showing all causal paths is more useful than picking one arbitrarily, and dedup can happen visually in the report.
...
[Answer]: Correct — this ticket keeps the legacy run-scoped marker write/read path with no sunset date; full removal is out of scope and deferred to a separate follow-up ticket once consumers have migrated. No deprecation-warning artifact is required in this ticket's scope.
...
[Answer]: Yes, that is sufficient — 100+ randomly generated DAGs up to 50 nodes, using vitest with a hand-rolled or fast-check-based generator (whichever is idiomatic for this repo existing TS test setup) is an acceptable bar for the property test.

...
- **Concept:** Allow DATUM to run autonomously using an embedded LLM client (`litellm`), removing the need for a host tool like Claude Code or WFC to drive the pipeline.
- **Risks:** Couples orchestration engine with inference execution. Adds complexity of context windows, API retries, and token limits to DATUM. Could end up poorly recreating WFC.
...
- **Shelved:** Await V2 stabilization before assimilating the `wfc-pm` and `wfc-ba` skill trees.

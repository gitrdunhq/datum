...
| G6 | Data plane | 0005/0013 | 🟡 **ledger deepened** — real SQLite ledger: run-scoping, token metering (`totals`/`tokens_spent`), richer records, file persistence. Valkey resume + per-run DB branch remain. | real Valkey checkpointer + libSQL ledger + per-run DB branch + metering | ledger rows queryable + metered ✔; kill mid-run → resume from checkpoint (pending) | P2 |

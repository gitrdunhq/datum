...
| G6 | Data plane | 0005/0013/0031 | 🟡 **ledger deepened + pluggable** — real SQLite ledger (run-scoping, metering, persistence) **behind a `RunLedger` port** with a `build_ledger(url)` factory (centralized DB = config swap). Valkey resume + a real centralized adapter remain. | `RunLedger`/checkpointer ports with local + centralized backends; per-run DB branch | ledger metered + swappable ✔; kill mid-run → resume from checkpoint (pending) | P2 |

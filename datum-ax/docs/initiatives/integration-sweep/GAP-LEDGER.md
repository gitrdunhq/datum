...
| G12 | Persona registry for Roles + Skills | 0033/0004/0020/0003 | ✅ **done (ADR-0033):** `Role`/`Skill` shapes + `PersonaRegistry` port + `FilePersonaRegistry` (markdown+frontmatter, deterministic, registered in `PERSONA_REGISTRIES`) + `build_persona_registry` factory. Crane owns the registry and `compose_system(role_id, scope_tags)` fills the old `BASE_PERSONA & SKILL_PERSONA_HERE` seam; `prompts/*.md` migrated to packaged `personas/roles/*.md`; triage/lane_plan/synthesis source their prompt from the registry via the crane. The GitNexus code-intelligence suite + bug-hunt (7 skills) were imported from the datum skill library into `personas/skills/*.md` (tagged `code-intelligence` + specifics). | per-lane Skill *selection* into prompts (lane `scope_tags` → `select_skills`); semantic role matching (cognition-side, optional); harvested rules delivered as Skills (ADR-0020). | port conformance + deterministic select ✔; crane composes Role into the prefix ✔; 7 datum skills imported + selectable ✔ | P3 |

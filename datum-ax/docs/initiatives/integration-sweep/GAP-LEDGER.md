...
| G12 | Persona registry for Roles + Skills *(investigate)* | 0004/0020/0003 | placeholder only — `crane.pack_payload` hardcodes `BASE_PERSONA & SKILL_PERSONA_HERE` in the `[System]` prefix; Roles live as bare `prompts/*.md` (no frontmatter/metadata), role→model in `ModelRoleRegistry`; **Skills unbuilt**, role selection is deterministic-by-phase (no semantic match). | a versioned, portable registry of Role + Skill artifacts (markdown + metadata) pulled **just-in-time** into the crane prefix; fills the persona/skill placeholder and backs ADR-0020's rules registry. **Spike:** evaluate [JasperHG90/persona](https://github.com/JasperHG90/persona) (markdown + Parquet embeddings, MCP server, `roles match`) as the implementation vs. building our own. | a lane's `[System]` prefix is composed from a registry-resolved Role + selected Skill(s), versioned and swappable; decision recorded in RESEARCH-NOTES | P3 |

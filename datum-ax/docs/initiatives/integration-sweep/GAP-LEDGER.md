...
| G12 | Persona registry for Roles + Skills | 0033/0004/0020/0003 | ✅ **done (ADR-0033):** `Role`/`Skill` shapes + `PersonaRegistry` port + `FilePersonaRegistry` (markdown+frontmatter, deterministic, registered in `PERSONA_REGISTRIES`) + `build_persona_registry` factory. Crane owns the registry and `compose_system(role_id, scope_tags)` fills the old `BASE_PERSONA & SKILL_PERSONA_HERE` seam; `prompts/*.md` migrated to packaged `personas/roles/*.md`; triage/lane_plan/synthesis source their prompt from the registry via the crane. Skills are wired through `select_skills` but no Skill artifacts authored yet. | semantic role matching (cognition-side, optional); authored Skills selected per-lane by scope_tags; harvested rules delivered as Skills (ADR-0020). | port conformance + deterministic select ✔; crane composes Role into the prefix ✔ (`prompts/` gone) | P3 |

...
| G12 | Persona registry for Roles + Skills | 0033/0004/0020/0003 | **decided (ADR-0033): build our own lean `PersonaRegistry` behind a port** (borrow Persona's markdown+frontmatter format, skip its Parquet/embeddings/TUI/MCP; keep selection deterministic). Not yet implemented — `crane.pack_payload` still hardcodes `BASE_PERSONA & SKILL_PERSONA_HERE`; Roles are bare `prompts/*.md`; Skills unbuilt. | `Role`/`Skill` shapes + `PersonaRegistry` port + `FilePersonaRegistry` adapter; crane resolves Role into the stable prefix and lane-Skills into the variable slot (ADR-0020 cache discipline). | a lane's `[System]` prefix is composed from a registry-resolved Role + tag-selected Skill(s), versioned and swappable | P3 |

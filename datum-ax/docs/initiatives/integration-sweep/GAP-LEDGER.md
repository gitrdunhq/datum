...
| G12 | Persona registry for Roles + Skills | 0033/0004/0020/0003 | 🟡 **slice 1 built (ADR-0033):** `Role`/`Skill` shapes + `PersonaRegistry` port (`contracts/persona.py`) + `FilePersonaRegistry` adapter (markdown+frontmatter, deterministic; registered in `PERSONA_REGISTRIES`) + `build_persona_registry` factory; conformance + property tests green. **Remaining:** crane integration (still hardcodes `BASE_PERSONA & SKILL_PERSONA_HERE`) + migrate `prompts/*.md` into `personas/`. | crane resolves Role into the stable prefix and lane-Skills into the variable slot (ADR-0020 cache discipline). | slice 1: port conformance + deterministic select ✔; slice 2: a lane's `[System]` prefix composed from a registry-resolved Role + tag-selected Skill(s) | P3 |

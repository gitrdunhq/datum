...
class NativeMlxTransport(OmlxTransport):
    ...
    async def complete(self, request: ChatRequest) -> ChatResponse:
        ...
        text = mlx_lm.generate(
            model, tokenizer, prompt=prompt, max_tokens=request.max_tokens, temp=request.temperature
        )

        # 4. Return standard datum-ax ChatResponse
        return ChatResponse(
            text=text, usage=Usage(input_tokens=0, output_tokens=0), finish_reason="stop"
        )

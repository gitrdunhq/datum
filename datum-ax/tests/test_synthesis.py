...
class MockInferenceClient:
    def complete(self, prompt, **kwargs):
        ...
        if "test" in str(prompt).lower():
            return Completion(text='{"diff": "--- a/test_foo.py\\n+++ b/test_foo.py\\n+def test_foo(): pass"}', model_id="mock", role=ModelRole.EXECUTOR, input_tokens=10, output_tokens=10)
        return Completion(text='{"diff": "--- a/foo.py\\n+++ b/foo.py\\n+def foo(): pass"}', model_id="mock", role=ModelRole.EXECUTOR, input_tokens=10, output_tokens=10)

...
First, decide on a short, descriptive, kebab-case name for the task or initiative (e.g., `tic-tac-toe-web`).
Write the result to `docs/initiatives/${INITIATIVE_NAME}/TICKET.md` (it becomes the GitHub epic issue body, ADR-0023). If it's a large initiative that produces an `INITIATIVE.md`, write it to `docs/initiatives/${INITIATIVE_NAME}/INITIATIVE.md`.
Then stop — REFINE and ROUTE take it from here. A human reviews/edits the ticket (scope is human-owned, ADR-0023) before the deterministic pipeline proceeds.

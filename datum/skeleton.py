...
def extract_skeleton(content: str) -> str:
    ...
    for _i, line in enumerate(lines):
        ...
        if (
            ...
            and skeleton
            and (
                skeleton[-1].lstrip().startswith("def ")
                or skeleton[-1].lstrip().startswith("class ")
                or skeleton[-1].lstrip().startswith("async def ")
            )
        ):
        ...
        if indent == 0 and stripped:
            ...
            if (
                not stripped.startswith("if __name__")
...
def get_git_frequency(path: str, repo_dir: str = ".") -> int:
    try:
        ...
        # Heuristic priority scoring only (not a gate/decision) — if git
        # is unavailable or the path isn't tracked, 0 is a reasonable
        # neutral contribution to the score.
        return 0
...
def get_import_fan_in(path: str, repo_dir: str = ".") -> int:
    ...
    try:
        ...
        # common case — no file imports this stem), and this is heuristic
        # priority scoring only, so 0 is the correct neutral result.
        return 0

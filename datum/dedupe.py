...
def fuzzy_match(f1: dict, f2: dict) -> bool:
    ...
    # Both findings must have explicit line numbers; if either lacks one,
    # we cannot reliably compare them and must treat as distinct findings.
    try:
        line1_str = f1.get("line")
        line2_str = f2.get("line")
        # If either finding lacks a line number, they're not duplicates
        if line1_str is None or line2_str is None:
            return False
        l1 = int(line1_str)
        l2 = int(line2_str)
        if abs(l1 - l2) <= 3:
        ...
        for existing in deduped:
            if fuzzy_match(finding, existing):
                ...
                if new_protected and not exist_protected:
                    ...
                    existing["severity"] = consolidate_severity(
